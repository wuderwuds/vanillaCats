...
class Api {
  ...
  async newCat(data) {
    return this.getRes(
      await fetch(`${this.url}/${this.userName}/add`, {
        method: "POST",
  ...
  async deleteCat(id) {
    return this.getRes(
      await fetch(`${this.url}/${this.userName}/delete/${id}`, {
        method: "DELETE",
      }),
    );
  }
...
async function deleteCat() {
  try {
    const response = (await api.deleteCat(7988)).json();
...
async function addCat() {
  try {
    await api.newCat({
      ...
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQOQUQ4vJLyAM14QgiCFjEon1BCRoJ_RTqXYA&usqp=CAU",
      age: 0,
      rate: 0,
      favorite: false,
      description: "",
